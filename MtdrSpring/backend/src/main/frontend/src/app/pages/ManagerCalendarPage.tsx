...
export default function ManagerCalendarPage() {
  ...
  const handleTaskClick = (task: Task) => {
    navigate(`/manager/task/${task.id}`, { state: { fromManagerCalendar: true } });
  };
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-white lg:pl-60">
        ...
        <Header2
          title="Calendar"
          subtitle="View all developer tasks by date"
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-7 gap-0 border border-slate-300 bg-white">
            {calendarData.days.map((day, index) => (
              <div
                key={index}
                className="aspect-square border-r border-b border-slate-300 p-2 bg-white hover:bg-slate-50 transition-colors overflow-visible flex flex-col min-h-24"
              >
                {day !== null ? (
                  <>
...
function TaskBox({ task, onClick, isHovered, onMouseEnter, onMouseLeave, developers }: TaskBoxProps) {
  ...
  const developerName = developers.find(d => d.id === task.assignedTo)?.name || 'Unassigned';
    ...
    <>
      <div className="relative z-0">
        <div
          ...
          title={`${task.title} (${developerName})`}
        >
      ...
      {/* Tooltip on hover - positioned absolutely to break out of overflow constraints */}
      {isHovered && (
        <div className="fixed z-50 bg-slate-900 text-white text-xs rounded px-2 py-1 whitespace-normal pointer-events-none shadow-lg max-w-xs" style={{
          top: '50%',
          left: '50%',
          transform: 'translate(calc(-50% + 30px), -50%)'
        }}>
          <div className="font-semibold">{task.title}</div>
          <div className="text-slate-300 text-[10px]">{developerName}</div>
        </div>
      )}
    </>
  );
